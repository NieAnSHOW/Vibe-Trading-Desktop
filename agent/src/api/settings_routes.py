...
def _read_settings_env_values() -> Dict[str, str]:
    ...
    """
    host = _host()
    env_path = host._resolve_settings_env_path()
...
# ---------------------------------------------------------------------------
# When the user switches LLM provider, the previous provider's API key / base
# URL must be dropped from the dotenv so stale credentials don't linger.
# upstream's _write_env_values only upserts; _rewrite_env_values drops the
...
def register_settings_routes(
    ...
    async def update_data_source_settings(payload: UpdateDataSourceSettingsRequest):
        ...
        if updates:
            host_ref._write_env_values(host_ref._resolve_settings_env_path(), updates)
    ...
    # fork: expose handlers on the host module so legacy tests that call
    # api_server.update_llm_settings(...) directly (not via HTTP) still work
    # after the route-extraction refactor.
    host.update_llm_settings = update_llm_settings

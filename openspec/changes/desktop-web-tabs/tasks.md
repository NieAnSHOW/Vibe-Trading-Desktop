...
- [x] 5.1 改 `resources.rs`：解析 `desktop-shell` 资源路径（dev 从源文件、release 从 bundle 资源），新增对应字段 + 单元测试
- [x] 5.2 改 `Cargo.toml`：`tauri` 正式开 `unstable` feature（spike 通过后）
- [x] 5.3 改 `tauri.conf.json`：`bundle.resources` 映射 `desktop-shell/` 资源；确认 `frontendDist`/`beforeDevCommand` 不被破坏
- [x] 5.4 （条件）若 §8 D9 降级路径启用，新增 `tauri-plugin-opener` 依赖
...
- [x] 7.1 Rust 单元测试：`open_news_tab` 幂等（重复 site_id 不重建，只切前台）
- [x] 7.2 Rust 单元测试：标签 label 唯一性
- [x] 7.3 Rust 单元测试：`close_tab` 拒绝关闭主页标签（closable=false）
- [x] 7.4 Rust 单元测试：`close_tab` 后再 `open_news_tab` 能正常重建；`open_grid_tab` 幂等
- [x] 7.5 Rust 单元测试：`open_app_tab` 创建固定首个标签

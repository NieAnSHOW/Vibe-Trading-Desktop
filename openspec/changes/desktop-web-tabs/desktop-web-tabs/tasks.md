...
- [ ] 1.1 `[spike]` 在 `Cargo.toml` 临时开 `tauri = { features = ["unstable"] }`，验证 Tauri 2.11.2 能否在 `main` 窗口内用 `Window::add_child` + `WebviewBuilder` 创建多个叠加 webview
- [ ] 1.2 `[spike]` 验证 macOS WKWebView 上多 webview 叠加 + `hide()`/`show()` 的实际行为（焦点抢占、z-order、显示隐藏切换）
- [ ] 1.3 `[spike]` 验证 Windows WebView2 上同上行为（若环境可用，否则标注仅 macOS 验证、Windows 待真机）
- [ ] 1.4 `[spike]` 验证窗口 resize 时多 webview 布局同步实现（全局监听 `WindowEvent::Resized` vs 每次 show 时重算）
- [ ] 1.5 `[spike]` 确定壳 webview 注入 `@tauri-apps/api` 的方式（`window.__TAURI__` 自动注入 vs UMD），及纯 HTML 路径下依赖管理
- [ ] 1.6 `[spike]` 决策点：多 webview 叠加可行则继续 §2 主路径；不可行则切到 §8 D9 外部浏览器降级路径

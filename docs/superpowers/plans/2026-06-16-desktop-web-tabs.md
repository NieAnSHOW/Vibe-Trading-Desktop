...
- [x] **Step 1: 写失败测试（先建文件含测试 + 最小类型签名占位）**
...
```
  ...
  - [x] **Step 2: 实现 register_app_tab（boot 内部调用，非 #[command]）**
...
```
  ...
  - [x] **Step 3: 实现 open_grid_tab（async，幂等）**
...
```
  ...
  - [x] **Step 4: 实现 open_news_tab（async，幂等外部站）**
...
```
  ...
  - [x] **Step 5: 实现 activate_tab + close_tab + hide_others**
...
- [x] **Step 5: 编译确认 capability schema 合法**
...
- [x] **Step 1: 改 imports 与 managed state**
...
```
  ...
  - [x] **Step 2: 改 setup —— 裸窗口 + 单 app webview 全窗加载**

...
- 缓存键 = (watchlist_version, since_cursor)；
- watchlist_version = sha256(sorted((code, market) pairs))；
- 自选增删 → 版本键变化 → 旧缓存自然失效；
...
上交所/深交所 ──5-10min──────▶ news.db            ◀── ~/.vibe-trading/ ── 键=(watchlist_version, watchlist-feed
  │                           (快讯24h/公告7d窗口)     watchlist.db（直读SQLite）  since_cursor)      ──▶ News 页统一流
  └─ 入库：URL 规范化 / 去重 / 保存结构化代码字段
```

采集器按各自增量游标（快讯 since_id、公告 5-10min 增量）拉取，入库执行 URL 规范化、去重并保存来源结构化代码字段；清理任务持续删除超窗条目；读取请求到达时，匹配器直读本机 watchlist.db 对窗口内条目执行三级匹配，结果按 (watchlist_version, since_cursor) 缓存；端点返回 FeedItem 流，前端 10-15s 短轮询消费。
...
```
GET /news-api/watchlist-feed?since=<cursor>&limit=50
```
...
- next_cursor: string|null——null 表示窗口内已无更多条目；cursor 为不透明字符串，客户端不得解析内部结构
...
- reset_required: boolean——见 6.1 游标重置说明

边界响应：

- 空自选列表：items=[]、next_cursor=null、reset_required=false、其余字段正常填充——前端显示引导添加自选的空状态
...
当请求中的 since_cursor 对应的 watchlist_version 与当前不一致时，服务端返回 reset_required=true 且 items 从窗口头部重新匹配，前端应丢弃旧 cursor 并以空 cursor 重新拉取。
...
| L182 | MUST NOT 定时刷新 / 资讯历史 | 允许后台轮询；保留禁止可浏览历史；留存窗口写入规格 |

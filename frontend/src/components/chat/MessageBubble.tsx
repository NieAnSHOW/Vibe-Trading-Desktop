...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      title={
        copied ? i18n.t("messageBubble.copied") : i18n.t("messageBubble.copy")
      }
    >
      {copied ? (
    ...
    return (
      <div className="flex gap-3 group">
        ...
        <div className="flex-1 min-w-0 relative rounded-2xl px-4 py-2.5 rounded-tl-sm border bg-card">
          ...
          {ts && (
            <span className="text-[9px] text-muted-foreground/30 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
              {ts}
            </span>
          )}
        </div>
